...
<div class="mt-10">
    <div class="flex justify-between items-center mb-2">
        <h1 class="text-xl font-bold">Edit Stagiaire</h1>
    ...
    <form action="{{route('stagiaires.update',$stagiaire->id)}}" method="post" enctype="multipart/form-data">
        ...
        <div class="flex bg-gray-900 justify-around items-center">
            <div class="p-5 rounded w-1/2">
                <h1 class="text-lg text-white font-bold">Edit Stagiaire</h1>
                <p class="text-gray-500 text-sm">Please fill in the form to edit the target stagiaire</p>
                <div class="text-sm space-y-2">
                    <div class="flex flex-col">
                        <!-- <label for="firstName" class="text-sm text-white ms-2">First Name</label> -->
                        <input type="text" value="{{$stagiaire->firstName}}" name="firstName" id="firstName" placeholder="First Name"
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="text" value="{{$stagiaire->lastName}}" name="lastName" id="lastName" placeholder="Last Name"
                            class="bg-gray-800 border border-gray-700 p-2 rounded text-white outline-none">
                    </div>
                    <div class="flex flex-col">
                        <!-- <label for="email" class="text-sm text-white ms-2">E-mail</label> -->
                        <input type="email" value="{{$stagiaire->email}}" name="email" id="email" placeholder="E-mail"
                            class="bg-gray-800 border border-gray-700 p-2 rounded text-white outline-none">
                    </div>
                    <div class="flex flex-col">
                        <!-- <label for="group" class="text-sm text-white ms-2">Group</label> -->
                        <input type="text" value="{{$stagiaire->group}}" name="group" id="group" placeholder="Group"
                            class="bg-gray-800 border border-gray-700 p-2 rounded text-white outline-none">
                    </div>
                    <div class="flex flex-col">
            ...
            <div class="flex bg-gray-800 justify-center p-5 rounded w-72 h-80 cursor-pointer hover:bg-[#1e2d46] items-center"
                onclick="document.getElementById('fileInput').click();">
                @if($stagiaire->image)
                <img src="{{ route('get-image', basename($stagiaire->image)) }}" alt="image" class="">
                @else
                <div class="text-center text-sm">
                    <p id="imageText">
                        This Stagiaire doesn't have an image yet
                    </p>
                    <img src="" alt="image" id="imagePreview" class="hidden">
                </div>
                @endif
            </div>
        </div>
